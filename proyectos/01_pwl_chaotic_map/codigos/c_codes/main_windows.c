...
long long _power;	// factor de conversion
...
void inicializa( int a, int b ){        
    ...
    _power = (long long)1 << _b;	// calculo de factor de conversion
}
...
long long pwl_custom( long long m1, long long m2, long long b1, long long a, long long b2, long long x ){
	long long r;
	long long zero = setNumber( 0.0 );
	if(x <= -a && -a < zero){
	    //printf("Caso 1\n");
		r = multTrunc( x, m1 ) + b1;
	}else if(-a < x && x < zero){
...
int main(int argc, char *argv[]){
	...
	long long x_n;       // Actual
    long long x_ni;		// Siguiente
	long long m1pf, m2pf, b1pf, apf, b2pf;
...
// gcc -o simulation main_windows.c -lm
// ./simulation 
